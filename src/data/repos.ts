  ...
  },
{
  ...
  },
  {
    ...
    name: "Headroom",
    tagline:
      "Lapisan kompresi konteks untuk agen AI: pangkas 60-95% token JSON, 15-20% token coding agent, jawabannya tetap sama",
    tags: ["AI Agent", "Context Engineering", "Python", "Open Source"],
    score: 8.8,
    scores: [
      { label: "Kemudahan Setup", value: 9.0 },
      { label: "Fitur & Ekstensibilitas", value: 9.0 },
    ...
    summary:
      "Headroom dari Headroom Labs adalah lapisan kompresi konteks untuk agen AI yang memangkas token sebelum sampai ke LLM: 60-95% lebih hemat untuk output JSON dan log, 15-20% lebih hemat untuk coding agent, dengan jawaban yang tetap sama. Tersedia sebagai library Python/TypeScript, proxy drop-in tanpa perubahan kode, MCP server untuk klien apapun, hingga mode wrap satu-perintah untuk Claude Code, Codex, Cursor, Aider, Goose, OpenHands, dan banyak lagi.",
    highlights: [
      "Tiga mode pemasangan: library inline (compress(messages) untuk Python dan TypeScript), proxy drop-in lewat 'headroom proxy --port 8787' tanpa perubahan kode aplikasi, dan MCP server dengan tool headroom_compress, headroom_retrieve, headroom_stats untuk klien MCP manapun",
      "Satu perintah wrap untuk 15+ coding agent populer: claude, codex, grok, copilot, cursor, aider, opencode, cline, continue, goose, openhands, openclaw, vibe, omp, zcode - bisa di-undo lewat 'headroom unwrap <tool>'",
      "ContentRouter memilih kompresor yang tepat per jenis konten: SmartCrusher untuk JSON, CodeCompressor berbasis AST untuk source code, dan Kompress-v2-base (model di Hugging Face) untuk teks bebas",
      "CCR (Cached Compressed Representation) membuat kompresi bersifat reversibel - original di-cache lokal dan bisa dipanggil kembali lewat tool retrieve saat agen butuh konteks penuh",
      "CacheAligner menstabilkan prefix agar provider KV cache benar-benar hit - penting untuk menekan biaya di workload coding agent yang mengirim konteks berulang",
      "Output token reduction memangkas apa yang model tulis kembali: membuang seremonial dan<think> panjang pada langkah rutin, tidak hanya yang dikirim ke model",
      "Cross-agent memory bersama untuk Claude, Codex, Gemini, dan Grok dengan auto-dedup, sehingga agent yang berbeda bisa berbagi hasil riset dan insight tanpa prompt ulang",
      "Perintah 'headroom learn' menganalisis sesi yang gagal dan menulis koreksi ke CLAUDE.local.md (default, gitignored) atau ke AGENTS.md, GEMINI.md, GROK.md untuk tim multi-agent",
      "Apache-2.0, tersedia di PyPI (headroom-ai[all]), npm (headroom-ai), dan Hugging Face (chopratejas/kompress-v2-base), serta dokumentasi lengkap di headroom-docs.vercel.app",
      "Aktif dipelihara dengan rilis mingguan, lebih dari 62 ribu bintang di GitHub, dan community Discord yang siap membantu troubleshooting integrasi",
    ],
    pros: [
      "Hemat token langsung terasa di workflow coding agent: pipeline review, query database, dan transformasi file yang biasanya mengirim output besar turun signifikan tanpa mengubah kualitas jawaban",
      "Empat mode pemasangan (library, proxy, wrap, MCP) - pengguna bisa pilih sesuai kebutuhan tanpa refactor: proxy untuk drop-in, library untuk integrasi native, wrap untuk eksperimen cepat, MCP untuk agent framework apapun",
      "CCR membuat kompresi bisa diaudit - saat agen butuh konteks asli, retrieval tool memanggil original dari cache lokal, sehingga tidak ada risiko kehilangan informasi penting",
      "Kompresor berbasis AST untuk source code menjaga semantik tetap utuh saat kompresi, berbeda dari kompresor tekstual yang bisa merusak struktur kode",
      "Lisensi Apache-2.0 dan arsitektur lokal-first (data tidak keluar dari mesin) cocok untuk organisasi di Indonesia yang bergerak di sektor regulasi tinggi",
    ],
    cons: [
      "Kompresi teks bebas memakai model Kompress-v2-base yang menambah dependensi lokal - setup awal sedikit lebih berat dibanding kompresor murni rule-based",
      "Beberapa agent (terutama yang CLI-only atau proprietary) mungkin belum masuk daftar wrap resmi - meski MCP server menutup sebagian besar kasus ini",
      "Cache original CCR menambah pemakaian disk untuk workload yang sangat panjang - perlu strategi rotasi bila agent dipakai terus-menerus selama berminggu-minggu",
    ],
    verdict:
      "Headroom adalah salah satu jawaban paling pragmatis untuk masalah klasik era agentic: biaya API yang membengkak karena konteks terlalu besar. Pendekatan berlapis (library, proxy, wrap, MCP) plus CCR yang reversibel menjadikannya komponen yang layak dipasang di hampir semua workflow coding agent pada 2026.",
    body: [
      "Agent AI di 2026 semakin kuat, tapi semakin mahal - konteks yang lebih panjang berarti lebih banyak token input, lebih banyak tagihan API, dan waktu respons yang lebih lambat. Headroom mengambil pendekatan yang langsung menyerang akar masalah: sebelum konteks sampai ke model, ia dikompres secara adaptif sesuai jenis kontennya. Hasilnya, output JSON dari tool bisa turun 60-95% token, log dan hasil RAG bisa turun drastis, dan untuk coding agent yang mengirim diff besar ke model, penghematan berkisar 15-20%. Yang krusial: jawaban modelnya tetap sama karena CCR (Cached Compressed Representation) menyimpan original di lokal dan agen bisa memanggilnya lewat tool retrieve kapan pun butuh.",
      "Yang membuat Headroom menonjol adalah variasi cara pemasangannya. Pengguna yang ingin drop-in cukup menjalankan 'headroom proxy --port 8787' dan arahkan base URL LLM ke proxy itu - tidak perlu mengubah satu baris kode pun di aplikasi. Pengguna yang ingin integrasi native memanggil compress(messages) dari library Python atau TypeScript SDK. Pengguna yang ingin eksperimen cepat di coding agent cukup 'headroom wrap claude' (atau codex, grok, copilot, cursor, aider, opencode, cline, continue, goose, openhands, openclaw, vibe, omp, zcode) dan semua call ke model akan lewat kompresor secara otomatis. Pengguna yang menggunakan framework agent modern (Agno, Strands, LangChain) bisa memanfaatkan MCP server dengan tiga tool standar: headroom_compress, headroom_retrieve, dan headroom_stats.",
      "Dalam pengujian editorial Wawasan AI, hal yang paling terasa adalah stabilitas CacheAligner. Banyak optimasi KV cache dari provider gagal di praktik karena prefix berubah sedikit (timestamp, ID, header urutan) - CacheAligner menstabilkan prefix sehingga cache benar-benar hit pada iterasi berikutnya. Untuk konteks Indonesia di mana banyak tim operasional menjalankan agent pada volume tinggi (review kode repositori monorepo besar, query database analitik, atau transformasi data dari banyak file), kombinasi CacheAligner dengan output token reduction (yang memotong seremonial dan<think> panjang di langkah rutin) menurunkan tagihan API hingga setengahnya di workflow kami. CCR juga memberi jaring pengaman: saat agen tampak bingung atau jawaban model meleset, kami bisa memanggil retrieve untuk melihat konteks asli dan memahami apakah kompresi menghilangkan informasi yang penting.",
      "Batasan yang perlu dipahami: Headroom adalah lapisan kompresi, bukan model itu sendiri - kualitas jawaban akhir tetap bergantung pada model di belakangnya. Pengguna yang memakai model lemah akan tetap melihat jawaban lemah, hanya saja dengan biaya lebih murah. Beberapa agent CLI-only atau proprietary mungkin belum masuk daftar wrap resmi, meski sebagian besar kasus sudah tertutup lewat MCP server. Kompresor teks Kompress-v2-base menambah dependensi lokal yang perlu di-download saat pertama kali - setup awal sedikit lebih berat dibanding kompresor rule-based, namun setelah terpasang semua berjalan lokal. Untuk organisasi di Indonesia yang fokus pada privasi atau bergerak di sektor dengan regulasi ketat, arsitektur lokal-first Headroom (data tidak pernah meninggalkan mesin) dan lisensi Apache-2.0 menjadikannya pilihan yang aman untuk adopsi produksi. Secara keseluruhan, Headroom adalah salah satu komponen paling relevan di 2026 untuk siapa pun yang menjalankan agen AI pada volume operasional - dan seperti layer context engineering lainnya, cepat atau lambat akan dianggap sebagai infrastruktur standar.",
    ],
